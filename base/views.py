...
class StatsImportAndExportView(generics.ListAPIView):
    def get(self, request, *args, **kwargs):
        ...
        if year is None:
            return Response({'error': 'Thiếu tham số year'}, status=status.HTTP_400_BAD_REQUEST)

        # Tính tổng tiền nhập cho từng tháng trong năm
        import_stats = ImportForm.objects.filter(
            created_date__year=year
        ).annotate(
            month=ExtractMonth('created_date')
        ).values('month').annotate(
            total_import=Sum('total')
        ).order_by('month')

        # Tính tổng tiền xuất cho từng tháng trong năm
        export_stats = ExportForm.objects.filter(
            created_date__year=year
        ).annotate(
            month=ExtractMonth('created_date')
        ).values('month').annotate(
            total_export=Sum('total')
        ).order_by('month')
        ...
        for i in range(1, 13):  # 12 tháng
            month_import = next(
                (item for item in import_stats if item['month'] == i), {'total_import': 0})
            month_export = next(
                (item for item in export_stats if item['month'] == i), {'total_export': 0})
...
class Top5PopularProductsView(generics.GenericAPIView):
    def get(self, request, *args, **kwargs):
        ...
        export_details = ExportDetail.objects.filter(form__created_date__gte=start_date, form__created_date__lt=end_date)
        ...
        for detail in export_details:
            ...
            quantity = detail.quantity
            price = detail.price * quantity  # Tính tổng giá trị của sản phẩm trong mỗi form
        ...
        sorted_products = sorted(product_data.items(), key=lambda x: x[1]['value'], reverse=True)
...
class TotalImport(generics.GenericAPIView):
    def get(self, request, *args, **kwargs):
        ...
        percentage = (increase / previous_month_total * 100) if previous_month_total != 0 else 0
...
class ToltalExport(generics.GenericAPIView):
    def get(self, request, *args, **kwargs):
        ...
        # Tính tổng tiền xuất của tháng hiện tại
        current_month_total = ExportForm.objects.filter(
            created_date__year=current_year,
            created_date__month=current_month
        ).aggregate(Sum('total'))['total__sum'] or 0

        # Tính tổng tiền xuất của tháng trước
        previous_month_total = ExportForm.objects.filter(
            created_date__year=previous_year,
            created_date__month=previous_month
        ).aggregate(Sum('total'))['total__sum'] or 0
